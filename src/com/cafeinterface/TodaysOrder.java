...
public class TodaysOrder extends JFrame implements ActionListener {
	Choice choiceempid;
	JTable table;
...
getContentPane().setBackground(new Color(192, 192, 192));
		...
		JLabel search=new JLabel("Search By Order Id : ");
		...
		try {
			 ...
			 cd1.rs=cd1.st.executeQuery("select * from OrderInfo");
			while(cd1.rs.next()) {
				//System.out.println("1");
				choiceempid.add(cd1.rs.getString("OrderId"));
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 if(e.getSource()==bsearch) {
			String query="select *from OrderInfo where OrderId='"+choiceempid.getSelectedItem()+"'";
			try {
